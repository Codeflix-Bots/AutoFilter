...
async def link_handler(Mbot, message):
    ...
    try:
        ...
        if url.endswith("="):
           dump_file=await message.reply_video(url[:-1],caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ #ʀᴇQᴜᴇꜱᴛʙᴏx1 ᴏꜰꜰɪᴄɪᴀʟ")
        else:
            dump_file=await message.reply_video(url,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ#ʀᴇQᴜᴇꜱᴛʙᴏx1 ᴏꜰꜰɪᴄɪᴀʟ")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   if ddinsta:
                      dump_file=await message.reply_video(content_value,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ #ʀᴇQᴜᴇꜱᴛʙᴏx1 ᴏꜰꜰɪᴄɪᴀʟ")
                   else:
                       dump_file=await message.reply_video(content_value, caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ #ʀᴇQᴜᴇꜱᴛʙᴏx1 ᴏꜰꜰɪᴄɪᴀʟ")
               except:
              ...
              #    await message.reply(meta)
                  ...
                  if meta_tag.ok:
                     ...
                     meta=re.findall(r'href="(https?://[^"]+)"', res['data']) 
                      ...
                      return await message.reply("Oops something went wrong")
                  try:
                     dump_file=await message.reply_video(meta[0], caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ #ʀᴇQᴜᴇꜱᴛʙᴏx1 ᴏꜰꜰɪᴄɪᴀʟ")
                  except:
                      com=await message.reply(meta[0])
                      await asyncio.sleep(1)
                      try:
                          dump_file=await message.reply_video(com.text,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ #ʀᴇQᴜᴇꜱᴛʙᴏx1 ᴏꜰꜰɪᴄɪᴀʟ")
                          await com.delete()
                      except:
                          pass
          ...
          #     await message.reply(tracemsg)
            ##optinal 
            await message.reply(f"400: Sorry, Unable To Find It  try another or report it  to @helpsarthak_bot")
            ...
            if 'downfile' in locals():
                os.remove(downfile)
            await message.reply("<a href='https://t.me/requestbox1_bot'>ᴜsᴇ ɴᴇᴡ ғᴇᴀᴛᴜʀᴇs</a>")
